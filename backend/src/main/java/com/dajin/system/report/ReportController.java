...
public class ReportController {
    ...
    public ApiResponse<?> monthly(@RequestParam(required = false) String month, HttpServletRequest request) {
        ...
        return ApiResponse.ok(db.list("select day,sum(order_count) order_count,sum(amount)+sum(processing_amount) amount,sum(amount) sales_amount,sum(processing_amount) processing_amount,sum(amount)+sum(processing_amount) turnover from ("
                + "select date(create_time) day,count(*) order_count,coalesce(sum(pay_amount),0) amount,0 processing_amount from sales_order where store_id=:s"
                + salesWhere(request,"sales_id") + " and date_format(create_time,'%Y-%m')=:month and status=1 group by date(create_time)"
    ...
    public ApiResponse<?> overview(@RequestParam(required = false) String timeType,@RequestParam(required = false) String startDate,@RequestParam(required = false) String endDate,@RequestParam(required = false) Long employeeId,@RequestParam(required = false) String category,HttpServletRequest request) {
        ...
        result.put("trend",db.list("select day,sum(order_count) order_count,sum(amount)+sum(processing_amount) amount,sum(amount) sales_amount,sum(processing_amount) processing_amount,sum(amount)+sum(processing_amount) turnover from ("
                + "select date(o.create_time) day,coalesce(sum(o.pay_amount),0) amount,count(*) order_count,0 processing_amount from sales_order o where o.store_id=:s and o.status=1"+range.sql("o.create_time")+scope+" group by date(o.create_time)"
